...
extern const unsigned int jumpingSound_sampleRate;
extern const unsigned int jumpingSound_length;
extern const signed char jumpingSound_data[];
# 6 "game.c" 2
...
void updateGame() {
 ...
 updateMario();
    ...
    shadowOAM[0].attr0 = player.row | (0<<13) | (0<<8) | (0<<14);
    shadowOAM[0].attr1 = player.col | (1<<14);
    shadowOAM[0].attr2 = ((0)<<12) | ((0)*32+(player.aniFrame * 2));

    updateCoin(&coin);
    shadowOAM[1].attr0 = coin.row | (0<<13) | (0<<8) | (0<<14);
    shadowOAM[1].attr1 = coin.col | (1<<14);
    shadowOAM[1].attr2 = ((0)<<12) | ((2)*32+(8));

    updateHeart(&heart);
    shadowOAM[2].attr0 = heart.row | (0<<13) | (0<<8) | (0<<14);
    shadowOAM[2].attr1 = heart.col | (1<<14);
    shadowOAM[2].attr2 = ((0)<<12) | ((2)*32+(10));

    shadowOAM[3].attr0 = 140 | (0<<13) | (0<<8) | (1<<14);
    shadowOAM[3].attr1 = 0 | (3<<14);
    shadowOAM[3].attr2 = ((0)<<12) | ((4)*32+(0));

    shadowOAM[4].attr0 = 138 | (0<<13) | (0<<8) | (0<<14);
    shadowOAM[4].attr1 = 43 | (1<<14);
    shadowOAM[4].attr2 = ((0)<<12) | ((10)*32+(livesUpdate * 2));

 for (int i = 0; i < 3; i++) {
        if (goombs[i].active == 1) {
            updateGoombas(&goombs[i]);
            shadowOAM[5+i].attr0 = goombs[i].row | (0<<13) | (0<<8) | (0<<14);
            shadowOAM[5+i].attr1 = goombs[i].col | (1<<14);
            shadowOAM[5+i].attr2 = ((0)<<12) | ((2)*32+(goombs[i].aniFrame * 2));
        }
    }
 ...
 for (int i = 0; i < 3; i++) {
        if (fballs[i].active == 1) {
            updateFBS(&fballs[i]);
            shadowOAM[11+i].attr0 = fballs[i].row | (0<<13) | (0<<8) | (0<<14);
            shadowOAM[11+i].attr1 = fballs[i].col | (1<<14);
            shadowOAM[11+i].attr2 = ((0)<<12) | ((2)*32+(6));
        }
    }
...
void updateMario() {
    ...
    if((!(~(oldButtons)&((1<<6))) && (~buttons & ((1<<6)))) && player.jumping == 0) {
        if (player.row < 18) {
            player.row = 19;
        }
        player.jumping = 1;
        player.rdel = 1;
        playSoundB(jumpingSound_data, jumpingSound_length, 0);
        player.row -= 18;
...
void spawnObstacles() {
 for (int i = 0; i < 3; i++) {
  if (gameTimer1 >= 10 && goombs[i].active == 0 ) {
   goombs[i].active = 1;
   gameTimer1 = 0;
   break;
  }
    ...
    for (int i = 0; i < 4; i++) {
  if (gameTimer2 >= 20 && fballs[i].active == 0 ) {
   fballs[i].active = 1;
   gameTimer2 = 0;
   break;
  }
